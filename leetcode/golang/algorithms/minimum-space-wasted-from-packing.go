...
func MinimumSpaceWastedFromPacking(packages []int, boxes [][]int) int {
	const mod = 1e9 + 7
	sort.Ints(packages)
	packagesLength := len(packages)
	cumulativeSizes := make([]int, packagesLength+1)
	for i := 1; i <= packagesLength; i++ {
		cumulativeSizes[i] = cumulativeSizes[i-1] + packages[i-1]
	}
	...
	for _, box := range boxes {
		...
		// If the biggest box can't fit, the supplier is worthless
...
func binarySearch(nums []int, target int) int {
	l, r := 0, len(nums)-1
	for l < r {
		mid := (l + r) >> 1 // shift binaries right, equals Math.floor(n / 2)
		if nums[mid] <= target {
			l = mid + 1
		} else {
			r = mid
...
// You have n packages that you are trying to place in boxes,
// one package in each box. There are m suppliers that each produce
// boxes of different sizes (with infinite supply).
// A package can be placed in a box if the size of the package is less
// than or equal to the size of the box.
//
// The package sizes are given as an integer array packages, where
// packages[i] is the size of the i'th package. The suppliers are given as
// a 2D integer array boxes, where boxes[j] is an array of box sizes that
// the j'th supplier produces.

// You want to choose a single supplier and use boxes from them such
// that the total wasted space is minimized. For each package in a box,
// we define the space wasted to be size of the box - size of the package.
// The total wasted space is the sum of the space wasted in all the boxes.

// * For example, if you have to fit packages with sizes [2,3,5] and
// the supplier offers boxes of sizes [4,8], you can fit the packages of
// size-2 and size-3 into two boxes of size-4 and the package with size-5
// into a box of size-8. This would result in a waste of
// (4-2) + (4-3) + (8-5) = 6.
...
// The total waste is
// (5-3) + (5-5) + (10-8) + (10-10) + (14-11) + (14-12) = 9.
